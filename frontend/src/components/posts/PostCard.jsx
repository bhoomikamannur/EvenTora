...
const PostCard = ({ post, onLike, isLiked, onDelete, onEdit, isAdmin, onUpdate }) => {
  ...
  // Log when post prop changes
  useEffect(() => {
    console.log('🎯 PostCard re-rendered with post:', { 
      id: post._id, 
      likes: post.likes, 
      isLiked: post.isLiked,
      isLikedProp: isLiked 
    });
  }, [post.likes, post.isLiked, post._id]);
  ...
  const handleLikeClick = () => {
    ...
    console.log('📊 Current post state:', { likes: post.likes, isLiked });
    onLike(post._id);
  };

  // Determine if this post is liked - check both the post object and the isLiked prop
        ...
        <div className="flex items-center gap-6 px-4 py-3 border-t border-gray-100">
          <button 
            ...
            className={`flex items-center gap-2 transition ${
              currentIsLiked ? 'text-red-500' : 'text-gray-600 hover:text-red-500'
            }`}
          >
